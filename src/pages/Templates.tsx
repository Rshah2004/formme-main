...
import WorkflowStrip from "../components/homePage/WorkflowStrip";
import NetworkGlobeSection from "../components/homePage/NetworkGlobeSection";
...
const HomePage = () => {
  useEffect(() => {
    ...
    const update = () => {
      ...
      const start = vh * 0.85;
      const end = vh * 0.35;
      let progress = (start - globeRect.bottom) / (start - end);
      progress = Math.min(1, Math.max(0, progress));
      const eased = progress * progress * (3 - 2 * progress);

      const globeOpacity = 1 - eased;
      const greenOpacity = eased;
      const globeTranslate = 20 * eased;
      const greenTranslate = 20 * (1 - eased);

      globe.style.opacity = String(globeOpacity);
      globe.style.transform = `translateY(${globeTranslate}px)`;
      globe.style.willChange = "opacity, transform";

      green.style.opacity = String(greenOpacity);
      green.style.transform = `translateY(${greenTranslate}px)`;
      green.style.willChange = "opacity, transform";
    };
    ...
    globe.style.transition = "opacity 200ms ease-out, transform 200ms ease-out";
    green.style.transition = "opacity 200ms ease-out, transform 200ms ease-out";
    update();
  ...
  return (
    <div className="relative w-full min-h-screen overflow-hidden">
      ...
      <main className="flex-grow"> 
        <HeroSection />
        <div className="post-hero bg-[#FAFAFA]">
          <PipelineSection />
          <WorkflowStrip />
          <NetworkGlobeSection />
